...
if ($_SERVER["REQUEST_METHOD"] === "POST" && isset($_POST["save"]) && $_POST["genre_name"] !== "") {
    # update the music with the corresponding id
    $genre_name = $_POST["genre_name"];
    $color = $_POST['genre_color'];

    $stmt = $mysqli->prepare("UPDATE genre 
                        SET   color = ? 
                        WHERE genre_name = ? ;");
    $stmt->bind_param("ss", $color,$genre_name);
    $stmt->execute();
    $q_update_genre = $stmt->get_result();
